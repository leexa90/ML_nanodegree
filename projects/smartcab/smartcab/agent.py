...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        self.epsilon = self.epsilon_o* 2.77**(-self.alpha*self.counter/50)
        #self.epsilon = self.epsilon- 0.05
        self.counter += 1
        #print 'ZZZZZZZZZZZZZZZZZZZ',self.epsilon
        if testing==True:
            self.epsilon = 0
            self.alpha = 0
        return None
    ...
    def get_maxQ(self, state):
        ...
        maxQ = [self.Q[state][x] for x in self.Q[state]]
        maxQ = max(maxQ)
    ...
    def choose_action(self, state):
        ...
        if self.learning is True:
            if random.random() <  self.epsilon:
                ...
                print 'XXX',random.random() , self.epsilon
            else:
                maxQ = self.get_maxQ(state)
                actions = [ x for x in  self.Q[state]  if   self.Q[state][x] == maxQ]
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning is True:
            alpha = self.alpha
            X = reward
            self.Q[state][action] =  (1-alpha)*self.Q[state][action] + alpha*X
        return 
